...
\section{Etat de l'art}\label{sec:eda}
	...
	\subsection{Application du cadre}\label{sec:appcadre}
		...
		\begin{longtable}{| p{0.8cm} | p{1.8cm} | p{2.5cm} | p{3cm} | p{2.4cm} | p{2.6cm} | p{2.6cm} | p{1.5cm} |}
        	...
        	\hline
        	\cite{Mostefai_et_al._2019} & - & Colère / Ennui / Peur / Frustration / Joie & Ensemble défini pour les événements, les actions, les objectifs, les statuts des objectifs + Fonctions pour la cartographie de chaque tendances d'action pour chaque état émotionnel (et fonction inverse), la cartographie de la tendance d'action pour chaque état émotionnel et pour chaque statut d'objectif & Style de jeu du joueur & Envoi d'un événement approprié à l'état émotionnel reconnu. L'adaptation se fait de manière ponctuelle, lorsqu'il y a plus d'états émotionnels négatifs expérimentés par le joueur que d'états émotionnels positifs. Utilisation de chaînes de Markov et matrices de transitions pour amener le joueur d'un état émotionnel à un autre (via un ou plusieurs événements dans le jeu) & Des éléments autres que l'état émotionnel sont pris en compte pour l'adaptation & Jeux sérieux (apprentissage)\\
        	\hline
        	\cite{noor_et_al._2009} & - & - & Préférences culturelles & - & - & Utilisation de l'API GoogleSocialGraphAPI pour identifier l'utilisateur et définir son profil. Plusieurs APIs et scripts open-sources osnt utilisés  pour extraire des données des différents réseaux sociaux. WordNet et Wikipedia sont utilisés pour le filtrage des données. Et les bases de connaissances YAGO et DBpédia sont employées pour catégoriser les données et récupérer les données concernant la culture avec une base de connaissance spécifique (CRM) & Jeux sérieux (culture)\\
        	\hline
        	\cite{yang_et_al._2018} & BioNomadix Wirless Sensors MP150 (GSR, ECG, EMG, RR) + capteurs de mouvement sur 3 axes + caméra (enregistre le visage du joueur) + capteurs de mouvement des zygomatiques & Colère / Ennui / Peur / Frustration / Joie & Reconnaissance via réseau de neurones à convolution (CNN) + Données annotées ajoutées à une base de données (DAG database erag.lip6.fr, base de données des auteurs) & Niveau du joueur pour ce jeu + Niveau de difficulté de la partie + Score final & - & - & Jeu vidéo (Fifa 2016)\\
        	\hline
        	\cite{yannakakis_et_al._2009} & Capteurs de pression & Amusement & Moyenne du temps de réponse, la variance de la pression sur la case et le nombre d'interactions avec la plate-forme. L'"amusement" est recalculé 3 fois au cours de la partie (à t=45s, t=60s et t=75s) & - & Des règles prédéfinies permettent l'augmentation / la diminution du nombre de cases jouables ou de la rapidité d'apparition / de disparition des points lumineux sur la plate-forme selon le niveau d'amusement détecté & - & Playware ("Bugs-smasher")\\
        	\hline
...
%			Le critère "méthodes pour la détection et pour la reconnaissance des états émotionnels" nous permet d'identifier d'une part les capteurs utilisés (s'ils existent).
%			Et d'autre part, les algorithmes utilisés pour la détection et la reconnaissance d'états émotionnels.
%			Comme nous visons à utiliser des capteurs pour notre projet, il est important pour nous de savoir si c%ertains capteurs reviennent souvent dans la littérature et s'ils sont performants pour ce type d'application.
